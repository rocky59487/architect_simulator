...
void AFrameResponseSpectrumActor::Tick(float DeltaTime)
{
    ...
    // U-13: modular reduction to keep float precision indefinite (see ModalShape).
    if (CurrentPhase > 1e5f && EnvelopeHz > 1e-6f)
    {
        CurrentPhase = FMath::Fmod(CurrentPhase, 1.f / EnvelopeHz);
    }
