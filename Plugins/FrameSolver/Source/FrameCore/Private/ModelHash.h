...
// PreparedSystem and solveLoad() checks before reusing a factorization.
// Moved out of FrameSolver.cpp's anonymous namespace so the opt-in supernodal lane
// (SnSolver.cpp / SnSession.cpp) can run the SAME reuse-validity guard without duplicating it.
// Pure POD over FrameModel; no Eigen. Logic is byte-identical to the previous in-file definition.
//
