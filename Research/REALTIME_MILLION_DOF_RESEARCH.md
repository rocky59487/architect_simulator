...
- **下一步 M3**(逼近/超越 CHOLMOD,差距 2.35x):① supernode **amalgamation**(zero-pad 放大 panel → dgemm 更接近 378 GFLOPS peak;fundamental 平均 7 列太碎,dgemm 在小 panel 退化)② **並行**(etree level-set 平行 supernode;目前單執行緒 left-looking,僅 panel 內 OpenBLAS 多緒,而 CHOLMOD 走 MKL 多緒)③ 多步 refinement 補 1e-9。column-based 版留作交叉驗 oracle。
