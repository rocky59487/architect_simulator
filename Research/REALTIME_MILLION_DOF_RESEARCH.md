...
- **下一步 M2/M3**:M2 把 sn_chol 併入 `run_supernodal.ps1` 掃描框架,在大規模(30k–100k)對標 CHOLMOD(小規模 factor 太快不代表趨勢);M3 supernode amalgamation(zero-pad 放大 panel → BLAS3 效率)+ 並行(etree level-set / OpenMP)+ 利用建築層狀 etree → 逼近/超越 CHOLMOD;iterative refinement 補大規模 residual。column-based 版留作交叉驗 oracle。
