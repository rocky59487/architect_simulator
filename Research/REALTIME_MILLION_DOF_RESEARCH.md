...
- **階段 3b(剩餘,supernodal 真正生產價值)**:**SnSession**(PIMPL,ctor reduceFF+analyze+factorizeSuperParallel **factor 一次** / solveFrame solveSuper **重用**,仿 HpSession)——stateless `solveLoadSupernodal` 每次重 factor,SnSession 才發揮 supernodal「factor-once + solve-many」(遊戲引擎每幀 solve 主場)+ standalone F58 + UE test enabled + gate 52。
