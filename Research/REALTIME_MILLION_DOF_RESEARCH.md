...
- **下一步 M3b**:etree(supernode-level)level-set 並行 factor;多步 iterative refinement 補 64k res 1.4e-9 → 1e-9(一步不夠,K 條件數)。amalgamation 程式碼保留(opt-in,並行下夠大 panel 才划算);union-loop 開銷該移進 analyze(算一次)。column-based 版留作交叉驗 oracle。
