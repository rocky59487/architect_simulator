...
>
> **Latest tag:** v0.5.2 (S-08 unit AS-36-u1, sprint in flight — PlaceKSetMember node-pair degeneration fix: bare `AActor` had no RootComponent so SpawnActor location was silently dropped, all K-set endpoints collapsed to origin-relative offsets, two K1 columns shared one node pair → LDLT singular → heatmap never spawned. Fix = `USceneComponent` root graft; ScenarioFixture +SC8/SC9 regression sub-checks; ExpectedUeTests 149 unchanged) — see [`docs/RELEASE_v0.5.2.md`](RELEASE_v0.5.2.md) + [`docs/HANDOFF_v0.5.2.md`](HANDOFF_v0.5.2.md) + [`docs/logs/S-08/manager.md`](logs/S-08/manager.md). **user-driven PIE P10/P11 human re-verify pending** (fix affects all K-set placement paths; the S-06 P10/P11 claim could not have been true before v0.5.2).
> **Prior tags this minor:** v0.5.1 (S-07 close — AS-35 PIE auto-smoke 6-leg gate: `ArchSim.PIE.PortalFrameSmoke` + `Scripts/run_pie_gate.ps1`; USER-DRIVEN P1..P15 still canonical for「ready for student trial」per `pie-auto-smoke-preference` memory) / v0.5.0 (Sprint S-06 close — AS-30 + U-ALS + U-IWYU + U-LOW) / v0.4.0.1 (S-05 patch — AS-28 Scenario widget cross-world hotfix + IWYU first-header rebuild fix) / v0.4.0 (Sprint S-05 close — Scenario MVP minor bump; marked prerelease because PIE end-to-end flow broken by cross-world bug pre-v0.4.0.1) / v0.3.1 (Sprint S-04 patch close — S-03 carryover AS-20 + AS-24 + 7 cosmetic NITs + outside-repo hook fix) / v0.3.0 (Sprint S-03 close — hardening + PIE-world test harness foundation) / v0.2.0 (Sprint S-02 close — ALS pawn end-to-end) / v0.1.5 / v0.1.4 (patch bundles)
