...
- v0.6.1: S-09 additions (+12 headless leg-2 tests total) — **AS-40** `ArchSim.Persistence.ResetClearsComponentFlags` / `RegisterMemberNonFinite` / `ReplayOrphanGuard` / `SaveLoadGuards` (4 tests; persistence edge-case contracts: Reset clears flags, non-finite endpoint guard, orphan destroy-actor invariant, SaveToSlot empty/partial guards + LoadFromSlot pre-check); **AS-41** `ArchSim.Persistence.SpudV2LibraryStructRoundtrip` / `SpudV2RestoreLibraries` / `SpudV2InjectLoads` / `SpudV2FixityApi` / `SpudV2FormatVersion` / `SpudV2DeactivatedSaveGuard` / `SpudV2V1CompatDefaults` / `SpudN00TensionReleaseWire` (8 tests SC19–26; sidecar v2 format roundtrip: library defs / loads-UDLs injection / per-DOF fixity API / format-version stamp / deactivated-member guard / v1-compat skip-and-default; SC17 `ReplayOrphanDataInvariant` renamed `ReplayOrphanGuard`); **AS-42** PIE leg-6 収緊 (no new leg-2 tests; `run_pie_gate.ps1` per-test `$ExpectedPieTests` guard + screenshot freshness check + PortalFrame 180-tick hard assert + `SaveLoadSmoke` tracked-set oracle `SC_D` + empty-overwrite `SC_E1` direct PIE assert [VERIFIED]; `SC_E2` partial-snapshot / `SC_E3` orphan PIE direct [NEW CODE, fault-injection unreachable, DEFERRED]). See `docs/logs/S-09/` for full agent logs.
