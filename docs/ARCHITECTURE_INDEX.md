...
| AS-30 | Scenario valid-frame fixture + boundary support API | 🟡 backlog (HIGH; v0.5.0 主軸) — K1/K2/K4 placed at default smoke coordinates produce 12-DOF-free mechanism → LDLT rejects model → no HeatmapActor → PIE smoke P10/P11 FAIL. Widget needs `PlaceFixedSupport(FVector)`, node-snap logic, `SpawnDefaultPortalFrame()`. See [`docs/HANDOFF_v0.4.0.1.md`](HANDOFF_v0.4.0.1.md) "AS-29" entry (mis-numbered there; this is the actual ID). | docs/HANDOFF_v0.4.0.1.md § "does NOT close" |
