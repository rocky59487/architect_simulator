...
| 2. UE automation | headless `FrameCore.*` tests | **98** tests w/ cuDSS, **96** without | UE-side mirrors of the standalone oracles across the same subsystems — the dual-build contract holds. v3.4.0 added 26 `FrameCore.UE.*` tests across the Karamba3D-parity numerical surface: **Phase 1 (3)** BuildAndValidate / InvalidMemberRef / LibraryPresets; **Phase 2 (5)** CantileverDisplacement / Reaction / MemberForce / PivotMarginFinite / DemandSummary; **Phase 3 (5)** SolveLinearCantilever / AnalysisModalCantilever (β=1.875 1st-mode freq rel<5e-2) / AnalysisBucklingColumn (cantilever Euler K=2 P_cr=π²EI/(2L)² rel<5e-2) / LoadCombineEnvelope / ReanalysisSolveCantilever; **Phase 4 (7)** SolvePDelta / TensionOnly / SizeOpt / BESO / Corotational / ArcLength / DynCollapse; **Phase 5 (6)** ShellDKQParity / ShellQM6 / ShellKSigma / ShellWarpAdmit / ShellCorotational / NMInteractionWire. None are `#if FRAMECORE_CUDA` gated, so a non-cuDSS build emits 96, not 94. v3.3.0 added 2 tests (`FFrameCoreUEActorStressMeshTest` U-03 + `FFrameCoreUEMarshalJsonTest` U-01). v3.2.1 added 8 `FrameCoreUE.*` Phase 6 a-h. v3.2.0 bumped 60→62. v3.1.0 bumped 59→60 with `FFrameCoreStressFieldTest`. v2.11.1-RC bumped 58→59 with `FFrameCoreGpuBacksubStrictTest`. v2.11 Phase 7 bumped 57→58 for `FFrameCoreGpuBacksubTest`. Both GPU tests are `#if FRAMECORE_CUDA` gated. |
...
Guard rails: `run_gate.ps1` hard-fails if fewer than `$ExpectedUeTests = 98` UE tests run (v3.4.0 bump for the 26 new `FrameCore.UE.*` Phase 1-5 tests stacked on top of the v3.3.0 72-test baseline; 96 on non-cuDSS builds — pass `-ExpectedUeTests 96`)
