...
Guard rails: `run_gate.ps1` hard-fails if fewer than `$ExpectedUeTests = 120` UE tests run (v3.5.0 bump for the 22 new `FrameCore.UE.*` Phase 1-8 visual-surface tests stacked on top of the v3.4 98-test baseline; 118 on non-cuDSS builds — pass `-ExpectedUeTests 118`). **v3.5.0 disclosure: the 120/118 counts are projected, not verified end-to-end this release session. The UE Editor build + gate run is the Z-01 first action in [`docs/HANDOFF_v3.5.0.md`](HANDOFF_v3.5.0.md#z-01-first-action--verify-gate-on-a-clean-checkout); engine source delta from v3.4.0 is 0 lines under `FrameCore/`, so Leg 1 standalone evidence carries forward and Legs 2-6 are projected based on the additive UE-only delta.**
