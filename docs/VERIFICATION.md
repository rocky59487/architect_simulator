...
| 2. UE automation | headless `FrameCore.*` tests | **72** tests w/ cuDSS, **70** without | UE-side mirrors of the standalone oracles across the same subsystems — the dual-build contract holds. v3.3.0 added 2 tests: `FFrameCoreUEActorStressMeshTest` (U-03: actor + UProceduralMeshComponent sigma-band box mesh; 1 PMC section / 44 vertices / 252 indices / colour-ramp ordering / re-entry safety / empty-field edge), `FFrameCoreUEMarshalJsonTest` (U-01: ComputeFromJsonModel matches in-code fixture rel<1e-6 + missing-file + malformed-JSON paths return default field). Neither is `#if FRAMECORE_CUDA` gated, so a non-cuDSS build emits 70, not 68. v3.2.1 added 8 `FrameCoreUE.*` tests Phase 6 a-h (MarshalSSBeam/ShellPlate/MultiMember + Robustness + EditorTabSpawner + ThetaRange + ZeroLoad + AxialColumn). v3.2.0 bumped 60→62 with `FFrameCoreUEBlueprintSmokeTest` + `FFrameCoreUEEditorSmokeTest`. v3.1.0 bumped 59→60 with `FFrameCoreStressFieldTest`; v2.11.1-RC bumped 58→59 with `FFrameCoreGpuBacksubStrictTest`; v2.11 Phase 7 bumped 57→58 for `FFrameCoreGpuBacksubTest`. Both GPU tests are `#if FRAMECORE_CUDA` gated. |
...
Guard rails: `run_gate.ps1` hard-fails if fewer than `$ExpectedUeTests = 70` UE tests run (v3.2.1 bump for the 8 new `FrameCoreUE.*` Phase 6 tests stacked on top of the v3.2.0 BlueprintSmokeTest + EditorSmokeTest pair; 68 on non-cuDSS builds — pass `-ExpectedUeTests 68`)
...
|---|---|---|
| **a.** 5-leg | `Scripts\run_gate.ps1 -RequireOpenSees` | standalone + UE (default 72 with cuDSS — pass `-ExpectedUeTests 70` without) + OpenSees + deep audit + CLI |
