...
param(
    ...
    [int]$ExpectedUeTests = 65,      # v3.2.0 Phase 6a: +3 FrameCoreUE marshal coverage (FFrameCoreUEMarshalSSBeamTest = SS beam UDL marshal incl. internal forces; FFrameCoreUEMarshalShellPlateTest = clamped plate shellsTop/shellsBot + 4-corner traversal + bIsTopLayer; FFrameCoreUEMarshalMultiMemberTest = 3-member frame, user-set IDs 100/200/300 preserved, governing == real id 100). v3.2.0 (Phase 3): +1 FFrameCoreUEEditorSmokeTest (Slate panel construct + OnComputeClicked under #if WITH_EDITOR). v3.2.0 (Phase 2): +1 FFrameCoreUEBlueprintSmokeTest (BP exposure smoke; F68 cantilever POD oracle rel<1e-5 float lossy). v3.1.0 (S11): +1 FFrameCoreStressFieldTest (UE F68 mirror + D/C interlock). v2.11.1-RC: +1 FFrameCoreGpuBacksubStrictTest (UE F67s mirror, fails on silent CPU fallback). v2.11 Phase 7: +1 FFrameCoreGpuBacksubTest (UE F67 mirror, smoke). Previously 57: S10 +1; supernodal opt-in +1; SnSession +1; shell K_sigma buckling +1; shell CR rotation invariance +1; warped MITC4 admit +1; shell knockdown +1; shell curvature guard +1. On a box without cuDSS (FRAMECORE_CUDA=0 build), both GPU tests compile out -- pass `-ExpectedUeTests 63` so the count guard matches.
