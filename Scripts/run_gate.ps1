...
param(
    ...
    [int]$ExpectedUeTests = 62,      # v3.2.0 (Phase 3): +1 for FFrameCoreUEEditorSmokeTest (Slate panel construct + OnComputeClicked under #if WITH_EDITOR). v3.2.0 (Phase 2): +1 for FFrameCoreUEBlueprintSmokeTest (FrameCoreUE BP exposure smoke; cross-checks USTRUCT marshal against engine POD on F68 cantilever fixture, rel<1e-5 float lossy). v3.1.0 (S11): +1 for FFrameCoreStressFieldTest (UE F68 mirror, member stress field oracle + D/C interlock vs ElasticAllowable). v2.11.1-RC: +1 for FFrameCoreGpuBacksubStrictTest (UE F67s mirror, fails on silent CPU fallback). v2.11 Phase 7: +1 for FFrameCoreGpuBacksubTest (UE F67 mirror, smoke). Previously 57: S10 +1; supernodal opt-in +1; SnSession +1; shell K_sigma buckling +1; shell CR rotation invariance +1; warped MITC4 admit +1; shell knockdown +1; shell curvature guard +1. On a box without cuDSS (FRAMECORE_CUDA=0 build), both GPU tests compile out -- pass `-ExpectedUeTests 60` so the count guard matches.
