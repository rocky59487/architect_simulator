...
param(
    ...
    [int]$ExpectedUeTests = 60,      # v3.1.0 (S11): +1 for FFrameCoreStressFieldTest (UE F68 mirror, member stress field oracle + D/C interlock vs ElasticAllowable). v2.11.1-RC: +1 for FFrameCoreGpuBacksubStrictTest (UE F67s mirror, fails on silent CPU fallback). v2.11 Phase 7: +1 for FFrameCoreGpuBacksubTest (UE F67 mirror, smoke). Previously 57: S10 +1; supernodal opt-in +1; SnSession +1; shell K_sigma buckling +1; shell CR rotation invariance +1; warped MITC4 admit +1; shell knockdown +1; shell curvature guard +1. On a box without cuDSS (FRAMECORE_CUDA=0 build), both GPU tests compile out -- pass `-ExpectedUeTests 58` so the count guard matches.
